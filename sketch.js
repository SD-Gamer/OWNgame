...
function setup() {
  ...
  playersRef.once("value",(data) => {
     maxId = data.numChildren() +1;
  })
  ...
  boy.scale = 0.7;
  boy.debug = true;
  boy.setCollider("rectangle",0,0,70,220);
...
function draw() {
  ...
  if(gameState === START){
     ...
     if(showLeaderBoard){
        ...
        for(var plr in allPlayers){
            text(allPlayers[plr].name + ":" + allPlayers[plr].score,200,dht);
            dht += 30;
        }
     }
   }
  if(gameState === LEVEL1){
     ...
     if(keyDown(RIGHT_ARROW)){
      ...
      spawnLedges();
      stonesG.setColliderEach("rectangle",0,0,500,400);
      ...
      if(boy.y > 800){
         ...
         spawnZombiesOnly();
         zombieG.setColliderEach("rectangle",0,0,100,150);
         //set collider for humans also
         spawnHumans();
